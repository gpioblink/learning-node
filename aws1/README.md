...
- これから先の目的は３つ。「AWS Lambdaなどのサービスを使ってサーバレスバックエンドを作るのがいかに簡単かを示すこと」「サーバレスのさまざまなアーキテクチャ、パターンを実装し、探求すること」「各章末の練習問題にチャレンジできるようにすること」

- この章では、「アップロードされた動画を入力して、さまざまな形式とビットレートにトランスコードするイベント駆動パイプライン」を構築する。このエンコードのワークフローは動画がS3バケットにアップロードされた際に自動的に開始される。

- Amazon Elastic Transcoderの無料枠は小さいので料金が発生するかもね。

- システムの主な要件は「トランスコードプロセスはアップロードされたソース動画を３種類の異なる解像度、ビットレートに変換する」「S3バケットをアップロード用とトランスコード用に２つ使う」「トランスコードが成功するたびに、ファイルについての情報が書かれたメールで通知が送られる（Amazon SNSを使用）」「動画メタデータを格納するJSONファイルが作られ、トランスコード済み動画に格納される」

- 今回使うAWSのサービスは、「Amazon S3(ファイル格納)」「Amazon Elastic Transcoder(動画変換)」「Amazon SNS(通知)」「AWS Lambda(カスタムコードのオーケストレーションと実行)」

- Amazon S3の無料利用枠では、毎月標準ストレージに5GBのデータを格納し、20000回のGETリクエスト、2000回のPUTリクエストを発行し、15GBのデータを送信することができる。

- AWS Lambdaはここでは３つ作る。その用途は、システム中の処理のとりまとめが必要な部分や、他のサービスでは直接実行できない部分の処理。「ファイルがアップロードバケットにアップロードされるたびに、ElasticTranscoderジョブを作って実行する」「トランスコード済み動画パケットにトランスコードされた動画が追加されるたびに、公開アクセスになるようファイルのアクセス権を変更する」「トランスコード済み動画パケットにトランスコードされた動画が追加されるたびに、動画を分析してメタデータファイルを作成しAmazon S3に新しくトランスコードされた動画ファイルを保存」

- ↑ユーザーがシステムとやり取りしないといけないのは最初の動画ステージだけになる

- Lambdaランタイムから呼び出されるハンドラは、`exports.handler = function(){}`。lambda関数は新しいファイルがパケットに書き込まれるとすぐにAmazon S3から呼び出される。パケット名とアップロードされたファイルのキーは、eventオブジェクトを通じてLambda関数に渡される。そのあとでAmazon Elastic Transcoderのジョブを準備すればよい。
